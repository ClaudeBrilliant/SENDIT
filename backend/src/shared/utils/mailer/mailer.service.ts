...
export class MailerService {
  ...
  async sendEmail(options: EmailOptions): Promise<void> {
    try {
      const mailOptions = {
        from: this.configService.get<string>('SMTP_FROM', 'no-reply@sendit.com'),
        to: options.to,
  ...
  async sendStatusUpdateEmail(to: string, parcel: ParcelInfo, newStatus: string): Promise<void> {
  ...
  async sendDeliveryNotification(to: string, parcel: ParcelInfo): Promise<void> {
